...
| `--with-n8n` | ติดตั้ง Docker (ถ้าไม่มี) + start n8n container + deploy hooks + load launchd agents + สร้าง account/API key + activate 10 workflows อัตโนมัติ | ต้องการ n8n automation layer — ใส่ `N8N_EMAIL` + `N8N_PASSWORD` ใน `~/.claude/.secrets/n8n.env` ก่อนรัน (setup.sh จะรอถ้าค่ายัง placeholder) |
