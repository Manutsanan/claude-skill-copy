...
- [Install](#install)
  ...
  - [Live library documentation (Context7 MCP)](#live-library-documentation-context7-mcp)
  - [Design source (Figma MCP)](#design-source-figma-mcp)
...
| **Figma** | โหลด design structure + thumbnail node + อ่าน/เขียน comment | `ux` (design source) / `sa` (requirement gathering) |
...
4. Load skill trigger vocabulary (~/.claude/memory/skill_trigger_vocabulary.md)
   └─ Thai phrase → skill mapping ที่เติบโตจาก corrections
5. Echo top 3-5 relevant entries ให้คุณเห็น
6. Conflict check — propose ขัด memory? → หยุดถามก่อน
7. Token efficiency gate        (quality gate compact + rtk proxy commands)
8. Match intent → decision matrix + vocabulary → เลือก skill
   └─ บรรทัดแรกของ response: `→ invoking \`[skill]\` ([reason ≤5 words])`
```
...
| "ทดสอบ / verify fix / ดูว่าทำงานไหม" | `verify` |
| "เปิด app / run dev server / screenshot" | `run` |
| "simplify / ลด code / หา duplication" | `simplify` |
| "review PR / code review / ดู PR #X" | `review` |
| "security review branch / audit before merge" | `security-review` |
...
| ช่วยดู / ช่วยทำ / ลอง / ปรับ / แก้ไข / เพิ่ม / เปลี่ยน (ไม่มี object) | **AskUserQuestion เสมอ** — ไม่เดา |

ไม่ต้องเรียก `/skill-name` เอง — พิมพ์ภาษาไทยปกติได้

> **Soft confirmation echo** — ทุกครั้งที่ Claude เลือก skill บรรทัดแรกจะขึ้นว่า `→ invoking \`sa\` (spec ไม่ชัด)` ถ้าผิด แก้ได้ทันที 1 คำ ก่อนเสียทั้ง turn
...
**10 save triggers** บังคับให้ save ระหว่างทำงาน (ไม่รอจบ session):
...
10. **Skill invoke ถูกแก้** ("ไม่ใช่ sa / ควรเป็น fe") → save `feedback_skill_trigger_*` ทันที; distill pipeline รวบเข้า vocabulary file
...
Figma MCP โหลด design structure โดยตรงเข้า context — `ux` ใช้อ่าน spacing/color/component จาก source of truth โดยไม่ต้อง screenshot และ `sa` ใช้อ่าน design comment เพื่อ gather requirement

**Install — เพิ่มใน `~/.claude.json` ที่ key `mcpServers`:**
```json
"figma": {
  "type": "stdio",
  "command": "npx",
...
ดู Figma API key ที่ figma.com → Settings → Personal access tokens

**Tools ที่ได้ (namespaced เป็น `mcp__figma__*`):**

| Tool | Required params | Returns |
|---|---|---|
| `add_figma_file` | `url` (Figma file URL) | Design structure + file_key loaded into context |
| `view_node` | `file_key`, `node_id` (`<n>:<n>`) | Thumbnail image ของ node นั้น |
| `read_comments` | `file_key` | ทุก comment ในไฟล์ |
| `post_comment` | `file_key`, `message`, `x`, `y` | โพสต์ comment ที่ coordinate |
| `reply_to_comment` | `file_key`, `comment_id`, `message` | Reply comment เดิม |

**Extract params จาก URL:**
- `file_key` — จาก `figma.com/file/<file_key>/...` หรือ `figma.com/design/<file_key>/...`
- `node_id` — จาก URL param `?node-id=<n>-<n>` → แปลง `-` เป็น `:` → `<n>:<n>`

**ROI ต่อ skill:**

| Skill | What improves |
|---|---|
| `ux` | `add_figma_file` โหลด design structure ก่อน design — spacing/color/component ตรงจาก source ไม่ต้องเดา |
| `sa` | `read_comments` gather requirement จาก design feedback โดยตรง |

**Graceful degradation:** ถ้า Figma ต้อง auth / URL ผิด → fallback ขอ user ส่ง screenshot + description
...
พิมพ์ภาษาไทยปกติ Claude detect intent จาก decision matrix + skill trigger vocabulary:

```
"วิเคราะห์ requirement หน้านี้ให้หน่อย"          → invoking `sa` (วิเคราะห์ requirement)
"ตรวจ security ของ auth flow"                  → invoking `sa` (security audit)
"ทำ UI ให้สวยขึ้น + responsive"                 → invoking `ux` (UI + responsive)
"เขียน composable นี้ให้หน่อย"                  → invoking `fe` (เขียน composable)
"ทำไม login ไม่ได้ — เจอ error 500"             → invoking `debug` (error + ไม่ได้)
"migrate native select เป็น USelect ทั้ง project" → invoking `migrate` (migrate ทั้ง project)
"audit project นี้ — perf + dependency"        → invoking `audit` (perf + dependency)
"ทำหน้าใหม่ list สินค้า มี filter + sort"        → invoking `sa` → ux → fe pipeline
```

บรรทัดแรกของทุก response ที่ invoke skill จะขึ้น `→ invoking \`[skill]\` (...)` เสมอ — ถ้าผิดพิมพ์แก้ได้ทันที

**กรณี phrase กำกวม** — Claude จะถามก่อน ไม่เดา:
```
"ช่วยดูหน่อย"  →  AskUserQuestion: วิเคราะห์ spec / ดู UI / แก้ code / debug?
"ปรับ"         →  AskUserQuestion: ปรับ UI (ux) / ปรับ logic (fe) / ปรับ spec (sa)?
```
