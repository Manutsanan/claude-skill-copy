...
| `--with-n8n` | ติดตั้ง Docker (ถ้าไม่มี) + start n8n container + deploy hooks + load launchd agents + สร้าง account/API key + activate 9 workflows (10 webhook endpoints) อัตโนมัติ | ต้องการ n8n automation layer — ใส่ `N8N_EMAIL` + `N8N_PASSWORD` ใน `~/.claude/.secrets/n8n.env` ก่อนรัน (setup.sh จะรอถ้าค่ายัง placeholder) |
| `--with-rtk` | ติดตั้ง `rtk-rewrite.sh` เป็น PreToolUse Bash hook | มี RTK CLI ≥ 0.23.0 — รีไรท์ shell commands เพื่อ token savings 60-90% |
| `--with-cmux` | ติดตั้ง cmux status hooks (Stop/StopFailure/UserPromptSubmit) | ใช้ cmux.app — แสดง working/done/error ใน sidebar |
...
hooks/
    ...
    ├── n8n-notify.sh            # Stop + StopFailure (async) → enriched payload (last_skill, checkpoint_phases, mem_files/bytes, FOLLOWUPS.md content+today) → 3 webhooks in parallel; State Store recomputes followups_text on every session end (real-time refresh, no wait for launchd)
    ...
    ├── n8n-followups-check.sh   # [launchd daily 08:58] reads FOLLOWUPS.md → POST /webhook/claude-state-store (updates Data Tables; n8n Schedule Trigger fires at 09:00 BKK and reads followups_text directly)
...
launchd/
    ├── com.claude.followups-check.plist  # daily 08:58 → n8n-followups-check.sh (updates Data Tables 2 min before n8n schedule trigger)
...
scripts/
    ...
    └── create-n8n-workflows.py  # Create + activate all 10 n8n workflows via API: 9 webhook + 1 Error Handler; FOLLOWUPS gets Schedule Trigger too; all workflows point errorWorkflow to Error Handler (auto-called by setup.sh --with-n8n)
mcp-guides/
...
| `hooks/check-memory-size.py` | SessionStart (async) | `systemMessage` when current project ≥ 30 entries (critical at 45); stale pipeline reminder when any `in_progress` checkpoint exists; distill idle suggestion when ≥ 14d since last `/distill-memory`; **auto-trims skill learnings ≥ 200 lines** — removes oldest entries, writes dated backup, no manual step needed |
| `hooks/n8n-prefetch.sh` | UserPromptSubmit (once per session) | Injects pipeline state · pending followups · type-check errors · last verify/debug outcome · modified files from n8n state store as `additionalContext` (memory alerts excluded — already covered by `check-memory-size.py` systemMessage) |
...
**Memory-cap guardrail + proactive reminders** — `check-memory-size.py` runs at SessionStart (async) and emits `systemMessage` warnings in three cases: (1) current project memory crosses **30 entries** (critical at **45**) — MEMORY.md truncates after 200 lines so oversized memory degrades Phase 0 echoes silently; (2) any `in_progress` phase checkpoint exists from a prior session — surfaces the stale pipeline so you resume instead of restarting; (3) `/distill-memory` hasn't been run in **14 days** and total memory count is above 10 but below the count-based threshold — a time-based nudge that fires before things get noisy, not after. **Skill learnings auto-trim** — when any skill's `learnings.md` reaches **200 lines** (critical), the hook automatically removes the oldest entries (sorted by `**Date:**` field) until the file is ≤ 140 lines, writes a dated backup (`learnings.trimmed.YYYY-MM-DD.md`) before touching anything, and reports the result in the systemMessage. Warn threshold (150 lines) still requires `/distill-memory` manually — auto-trim only fires at critical to avoid silent data loss on healthy files.
...
**n8n automation layer** — hooks are stateless (fire-and-forget); n8n is the stateful middleware. 10 workflows (9 webhook endpoints + 1 Error Handler) extend the skill system without touching Claude's context: pipeline phase tracking (sa→ux→fe completion across sessions — `stop_type: failure` payloads are ignored so a crashed session never falsely advances phases), type-check trend history (regression detection, errors persisted to Data Tables and surfaced in next-session prefetch), memory growth monitoring, FOLLOWUPS.md deadline alerts, weekly skill analytics, repo sync drift detection, cross-project pattern promotion reminders, a **state store** backed by a persistent Data Table (`project_state`) that caches pipeline state, pending followups, type-check errors, last verify/debug outcome, and session-modified files — queried by `n8n-prefetch.sh` at session start to inject structured context before Phase 0 even runs (memory alerts excluded from injection — already covered by `check-memory-size.py`) — and a **vocab tracker** that accumulates skill-trigger corrections and fires a Telegram alert when the same phrase is corrected ≥ 2 times (recurring mismatch signal). **Error Handler** (workflow #10) uses n8n's Error Trigger to catch all workflow execution failures and sends a Telegram notification with the workflow name and error message — all other 9 workflows have `settings.errorWorkflow` pointing to it. **FOLLOWUPS Reminder** has a second trigger: a Schedule Trigger (daily 02:00 UTC = 09:00 Bangkok) that reads `followups_text` from the `project_state` Data Table directly, so the Telegram alert fires even without an active session — the launchd plist (`n8n-followups-check.sh`, 08:58) is now a safety-net fallback for days without any Claude session — `n8n-notify.sh` already reads FOLLOWUPS.md on every session end and pushes `content + today` to State Store so `followups_text` stays current in real-time. n8n runs in Docker (`~/.n8n` mounted only) — host scripts do all file reads and push pre-processed payloads via HTTP POST. Telegram remains notification-only and can be removed without affecting skill flow. **Setup is fully automated via `--with-n8n`:** Docker is installed if missing, n8n container is started, and a Playwright headless script (`n8n-setup-account.py`) creates the owner account, generates an API key, and writes it back to `n8n.env` — then `create-n8n-workflows.py` creates the `project_state` Data Table (with all required columns), creates and activates all 10 workflows (Error Handler first so its ID can be injected into all others), and injects the live table ID into node code automatically. The only manual step is setting `N8N_EMAIL` + `N8N_PASSWORD` in `~/.claude/.secrets/n8n.env`; setup.sh pauses and waits if they're still at placeholder values.
